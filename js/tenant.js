...
function displayTenants() {
    ...
    tenants.forEach((tenant, index) => {
        const tenantCard = document.createElement("article");
        ...
        tenantCard.innerHTML = `
            ...
            <p><strong>Rent:</strong> KES ${tenant.rent}</p>
...
tenantForm.addEventListener("submit", function(event) {
    ...
    if (tenantName.value.trim() === "") {
        nameError.textContent = "Tenant name is required";
        isValid = false;
    }

    if (apartmentName.value.trim() === "") {
        apartmentError.textContent = "Apartment name is required";
        isValid = false;
    }
    ...
    if (rentAmount.value.trim() === "") {
        rentError.textContent = "Rent amount is required";
        isValid = false;
    }
